...
Follow the **Pre-Execution** procedure in `.claude/skills/_common/state-logging.md`. The intent is "optimize $TARGET ($GOAL_TYPE)"; the approach is "profile, parallel bottleneck analysis, incremental optimization with measurement".
...
- **model**: `sonnet` (structured profiling analysis — see `.claude/skills/_common/model-selection.md`)
...
Follow the **Post-Execution** procedure in `.claude/skills/_common/state-logging.md`. Complete the DECISIONS entry with the outcome (baseline → final measurement, percentage improvement, optimizations applied vs reverted).

## 6. Auto-Commit and Auto PR

Follow the procedure in `.claude/skills/_common/auto-finalize.md`. Stage all modified source files. Commit: `perf: {summary of optimization and improvement}`. PR title should summarize the optimization.
