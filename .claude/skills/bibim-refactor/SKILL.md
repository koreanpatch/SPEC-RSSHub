...
- **model**: `sonnet` (scope analysis is grep + read — see `.claude/skills/_common/model-selection.md`)
...
Follow the **Pre-Execution** procedure in `.claude/skills/_common/state-logging.md`. The intent is `$REFACTOR_GOAL`; the approach is the migration plan from Step 2 (key waves and strategy).
...
If the wave has multiple independent steps, first classify their complexity per `.claude/skills/_common/model-selection.md`:
...
Follow the **Post-Execution** procedure in `.claude/skills/_common/state-logging.md`. Complete the DECISIONS entry with the outcome. Log any incidents (test failures per wave, coherence issues, conflicting agent changes).

## 5. Auto PR

Follow the procedure in `.claude/skills/_common/auto-finalize.md`. The PR title should summarize the refactoring (e.g., "refactor: move X from module A to module B"). Include a summary of what was changed across all waves.
