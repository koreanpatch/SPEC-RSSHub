...
Follow the procedure in `.claude/skills/_common/worktree-guard.md` with `$SKILL_NAME=tag-audit`.
...
Follow the **Pre-Execution** procedure in `.claude/skills/_common/state-logging.md`. The intent is "audit $TARGET_FOLDER for missing POS tags"; the approach is "prepare unique-word batches, parallel agent analysis with Sejong refs, consolidated patch".
...
Follow `.claude/skills/_common/codebase-context.md` to build `$CODEBASE_CONTEXT`.

Follow `.claude/skills/_common/korean-linguistic-context.md` to build `$LINGUISTIC_CONTEXT` from the standard reference docs (SEJONG_REFERENCE.md, STEM_TAGS.md, GLOSSARY.md, pos_mapping.yaml, irregulars.yaml).

## 2.5. Classify Batch Complexity

Follow the **Classifier Pattern** in `.claude/skills/_common/model-selection.md`. Spawn a single **haiku** agent with the manifest (source files, POS categories per batch). It returns a model assignment per batch:
...
ALL agents MUST be launched in a SINGLE message (true parallelism). Follow `.claude/skills/_common/agent-rules.md`.
...
Follow the **Post-Execution** procedure in `.claude/skills/_common/state-logging.md`. Complete the DECISIONS entry with the outcome (N findings: N high-confidence, N medium-confidence; across M target files).

## 6. Auto-Commit and Auto PR

Follow the procedure in `.claude/skills/_common/auto-finalize.md`. Stage batch files, findings, and the consolidated patch. Commit: `audit: tag audit of $TARGET_FOLDER — N findings across M files`.
