...
Follow the procedure in `.claude/skills/_common/worktree-guard.md` with `$SKILL_NAME=gap-analysis`.
...
Follow the **Pre-Execution** procedure in `.claude/skills/_common/state-logging.md`. The intent is "run gap analysis pipeline and evaluate findings"; the approach is "5-wave script execution + linguistic evaluation + disposition TSV update".
...
Follow the **Post-Execution** procedure in `.claude/skills/_common/state-logging.md`. Complete the DECISIONS entry with the outcome (N findings: N unambiguous, N ambiguous, N uncertain; N skipped per disposition). Log incidents for script failures or surprising evaluation results that changed understanding.

## 7. Auto-Commit and Auto PR

Follow the procedure in `.claude/skills/_common/auto-finalize.md`. Stage all changed files (reports, disposition TSV, parquets). Commit: "gap-analysis: N findings — N unambiguous, N ambiguous, N uncertain". PR title should summarize findings.
