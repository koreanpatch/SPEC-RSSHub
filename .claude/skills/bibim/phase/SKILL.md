...
Follow the **Pre-Execution** procedure in `.claude/skills/bibim/_common/state-logging.md`. The intent is `$PHASE`; the approach is the phase lifecycle (research → implement → test → docs).
...
Then follow the setup procedure in `.claude/skills/bibim/_common/worktree-setup.md`.
...
Follow the **Post-Execution** procedure in `.claude/skills/bibim/_common/state-logging.md`. Complete the DECISIONS entry with the outcome. Log any incidents (test failures that needed fixing, abandoned approaches, discovered bugs).

## 7. Auto PR

Follow the procedure in `.claude/skills/bibim/_common/auto-finalize.md`. The PR title should summarize the phase (e.g., "feat: implement {$PHASE}"). Include a summary of research, implementation, and test fixes in the PR body.
