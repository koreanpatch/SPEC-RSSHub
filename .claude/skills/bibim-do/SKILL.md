...
Follow the **Pre-Execution** procedure in `.claude/skills/_common/state-logging.md`. The intent is derived from `$FILE` and its pending tasks; the approach is wave-based parallel execution.
...
Follow the **Classifier Pattern** in `.claude/skills/_common/model-selection.md`. Spawn a single **haiku** agent with all pending task descriptions. It returns a model assignment per task:
...
Follow the procedure in `.claude/skills/_common/pragmatist-review.md`. Focus on cross-task coherence, codebase fit, and whether a better synthesis of the task outputs exists.
...
Follow the **Post-Execution** procedure in `.claude/skills/_common/state-logging.md`. Complete the DECISIONS entry with the outcome. Log any incidents (verification failures, agent conflicts, tasks that couldn't be completed).

## 7. Auto PR

Follow the procedure in `.claude/skills/_common/auto-finalize.md`. The PR title should summarize the work (e.g., "feat: implement 4 tasks from plan-xyz").
