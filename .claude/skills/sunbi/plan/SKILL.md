...
Follow the procedure in `.claude/skills/sunbi/_common/worktree-guard.md` with `$SKILL_NAME=plan`.
...
Follow the procedure in `.claude/skills/sunbi/_common/codebase-context.md` to build a `$CODEBASE_CONTEXT` block. Also read `.planning/STATE.md` for current progress. Focus on conventions and architecture relevant to the issue.
