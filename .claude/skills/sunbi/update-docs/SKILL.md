...
Follow the procedure in `.claude/skills/sunbi/_common/worktree-guard.md` with `$SKILL_NAME=update-docs`.
...
# Also: CLAUDE.md at project root (if present)
```
...
git log -1 --format="%ct" -- "src/" "*.ts" "*.tsx"
```
...
- `entrypoints.md` — verify the four WXT entrypoints and isolation rules match `src/entrypoints/`
- `store-shape.md` — verify Zustand fields/actions vs `store.ts` / `dictionaryStore.ts`
- `message-types.md` — verify against `src/shared/types/messages.ts`
- `routing.md` — verify `ViewId`, `NAV_ITEMS`, `VALID_VIEW_IDS`, `App.tsx` switch

**For `.planning/reference/` docs:**

- Verify module descriptions match current TypeScript sources under `src/`

**For root-level docs:**

- Project overview / architecture — verify build commands (`pnpm`), WXT layout, and ports
...
Follow the procedure in `.claude/skills/sunbi/_common/pragmatist-review.md`. Focus on cross-doc consistency, voice consistency, and missing cross-references across the `.planning/` documentation set.
...
- After all docs are updated and verified, auto-commit following `.claude/skills/sunbi/_common/auto-commit.md`. If pre-commit hooks modify files, re-stage and retry once.
</rules>
