...
Follow the procedure in `.claude/skills/sunbi/_common/worktree-guard.md` with `$SKILL_NAME=refactor`.
...
Follow the procedure in `.claude/skills/sunbi/_common/codebase-context.md` to build a `$CODEBASE_CONTEXT` block. Focus on conventions and rules that matter for the refactoring.
...
- Instructions:
    ...
    3. Check `docs/dev/README.md` and `docs/dev/architecture/overview.md` first for layout and entrypoints
...
- Instructions:
    ...
    3. Check `AGENTS.md` and `docs/dev/features/` for documented patterns
...
- **Step 1a**: {what to do} — Files: `path/to/Component.tsx`
- **Step 1b**: {what to do} — Files: `path/to/other.ts`

### Wave 2: {title}

{Steps that depend on Wave 1 output}

- **Step 2a**: {what to do} — Files: `path/to/file.ts`
...
- Test command to run after each wave: `pnpm vitest run` (and `pnpm typecheck` when types shift)

## Files Affected

| File                                    | Action | Wave | Purpose           |
| --------------------------------------- | ------ | ---- | ----------------- |
| `src/entrypoints/sidepanel/foo/Bar.tsx` | Modify | 1    | Move subcomponent |
```
...
Run `pnpm vitest run`. If tests fail:
...
After tests pass, follow the auto-commit procedure in `.claude/skills/sunbi/_common/auto-commit.md`. Stage only the changed files. Commit with a descriptive message (e.g., `refactor: move X from module A to module B`). If pre-commit hooks modify files, re-stage and retry once.
...
1. Run `pnpm vitest run` (and `pnpm typecheck` if needed) one more time to confirm everything is green
...
- ALWAYS run `pnpm vitest run` after each wave, not just at the end
...
- Plan file goes in `docs/plans/` (Sunbi convention)
...
- Use `pnpm` for verification — no Python venv unless a documented Python script is in scope
</rules>
