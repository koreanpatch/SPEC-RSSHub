...
Follow the procedure in `.claude/skills/sunbi/_common/worktree-guard.md` with `$SKILL_NAME=do`.
...
Follow the procedure in `.claude/skills/sunbi/_common/codebase-context.md` to build a `$CODEBASE_CONTEXT` block.
...
Follow the procedure in `.claude/skills/sunbi/_common/pragmatist-review.md`. Focus on cross-task coherence and codebase fit.
...
- After all tasks are complete and verification passes, auto-commit following `.claude/skills/sunbi/_common/auto-commit.md` with a descriptive message summarizing the work. Stage only the changed files with `git add`. If pre-commit hooks modify files, re-stage and retry once.
