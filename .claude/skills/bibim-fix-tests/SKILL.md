...
Follow the **Pre-Execution** procedure in `.claude/skills/_common/state-logging.md`. The intent is "fix test failures" (with `$TEST_TARGET` if specific); the approach is "diagnose in parallel, batch-fix, verify".
...
- **model**: `sonnet` (traceback analysis is structured — see `.claude/skills/_common/model-selection.md`)
...
Follow the procedure in `.claude/skills/_common/pragmatist-review.md`. Focus on cross-fix coherence, better alternatives, and side effects. Adjust fixes before re-running tests if issues are found.
...
Follow the **Post-Execution** procedure in `.claude/skills/_common/state-logging.md`. Complete the DECISIONS entry with the outcome (N failures fixed, N remaining). Log every root cause as an incident — fix-tests is the primary source of INCIDENTS entries.

## 6. Auto PR

Follow the procedure in `.claude/skills/_common/auto-finalize.md`. The PR title should summarize the fixes (e.g., "fix: resolve 3 test failures from missing jamo boundary handling").
