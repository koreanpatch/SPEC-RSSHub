...
Follow the procedure in `.claude/skills/dictionary/_common/worktree-guard.md` with `$SKILL_NAME=refactor`.
...
Follow the procedure in `.claude/skills/dictionary/_common/codebase-context.md` to build a `$CODEBASE_CONTEXT` block. Focus on conventions and rules that matter for the refactoring.
...
After tests pass, follow the auto-commit procedure in `.claude/skills/dictionary/_common/auto-commit.md`. Stage only the changed files. Commit with a descriptive message (e.g., `refactor: move X from module A to module B`). If pre-commit hooks modify files, re-stage and retry once.
