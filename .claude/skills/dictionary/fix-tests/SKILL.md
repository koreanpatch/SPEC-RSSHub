...
Follow the procedure in `.claude/skills/dictionary/_common/worktree-guard.md` with `$SKILL_NAME=fix-tests`.
...
Follow the procedure in `.claude/skills/dictionary/_common/pragmatist-review.md`. Focus on cross-fix coherence, proportionality, and side effects. Adjust fixes before re-running tests if issues are found.
...
- After all fixes are applied and tests pass, auto-commit following `.claude/skills/dictionary/_common/auto-commit.md`. If pre-commit hooks modify files, re-stage and retry once.
</rules>
