...
Follow the procedure in `.claude/skills/dictionary/_common/worktree-guard.md` with `$SKILL_NAME=profile`.
...
Follow the procedure in `.claude/skills/dictionary/_common/codebase-context.md` to build a `$CODEBASE_CONTEXT` block. Include the specific module/command/pipeline relevant to `$TARGET`.
...
Follow the procedure in `.claude/skills/dictionary/_common/pragmatist-review.md`. Focus on maintainability trade-offs and regression risk. If any optimization fails the pragmatist check, revert it and note it in the report.
...
- After all optimizations are complete and tests pass, auto-commit following `.claude/skills/dictionary/_common/auto-commit.md`
