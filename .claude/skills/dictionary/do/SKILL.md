...
Follow the procedure in `.claude/skills/dictionary/_common/worktree-guard.md` with `$SKILL_NAME=do`.
...
Follow the procedure in `.claude/skills/dictionary/_common/pragmatist-review.md`. Focus on cross-task coherence and codebase fit.
...
- After all tasks are complete and verification passes, auto-commit following `.claude/skills/dictionary/_common/auto-commit.md` with a descriptive message summarizing the work. Stage only the changed files with `git add`. If pre-commit hooks modify files, re-stage and retry once.
